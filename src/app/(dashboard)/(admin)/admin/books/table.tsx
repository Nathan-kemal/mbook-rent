...
const OwnersTable = () => {
  ...
  const handleApproveButton = async (userId: string) => {
    const response = await actionApproveBook(userId);
  ...
  const handleSwitchChange = async (
    ...
    try {
      ...
      const statusUpdate = await actionDisableBook(id, switch_staus);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = actionGetAllUploadedBook(params);
  ...
  const columns = useMemo<MRT_ColumnDef<Owner>[]>(
    ...
    () => [
...

      {
        id: "author",
        header: "Author",
        Cell: ({ cell }) => <Typography>{cell.row.original.author}</Typography>,
      },

      {
        ...
        header: "Category",
        Cell: ({ cell }) => (
          <Typography>{cell.row.original.category}</Typography>
        ),
      },
              ...
              <IconButton
                onClick={() =>
                  handleOpenOwnerInfo({
                    ...
                    phoneNumber: cell.row.original.User.phoneNumber,
                    location: cell.row.original.User.location,
                  })
                }
              >
                <VisibilityIcon />
